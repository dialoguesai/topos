    ...
    *,
    ...
    if only_missing:
        if job_name == "timeline":
            ...
            # Records this job has already RUN over, whatever it produced.
            # Coverage tables only witness output, so without this a record
            # that correctly yields nothing is re-scanned by every backfill
            # forever (see enrichment_record_progress_v1). Computed once —
            # this set is read per record, so building it inside the filter
            # would issue one query per row.
            already_run = _processed_record_ids(
                db_conn, source_id, job_name, catalog_spec_version(job_name)
            )
            if already_run:
                records = [r for r in records if _record_identifier(r) not in already_run]
    ...
    # Every record in `records` was attempted, so every one is marked — that is
    # the whole point, since `processed` above counts OUTPUT ROWS CREATED, not
    # records attempted, and the two differ by exactly the empty results this
    # marker exists to stop re-scanning.
    marked = _mark_records_processed(
        db_conn, source_id, job_name, records, catalog_spec_version(job_name)
    )
