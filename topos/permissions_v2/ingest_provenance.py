"""Owner-attested immutable iMessage and ChatGPT snapshots, confined to the isolated beta.
...
Each reader contract is one lane (``_LANES``): its fixed source id, attested
sentence, snapshot file form and the one canonical table its links name. The
durable tables are shared and unchanged, so an installed iMessage ledger keeps
its schema digest; a link's table is read from its enrollment's pinned snapshot
descriptor, never from the row it points at.
"""
...
READER_CONTRACT = IMESSAGE_READER_CONTRACT
...
def _lane(reader_contract):
    lane = _LANES.get(reader_contract) if type(reader_contract) is str else None
    if lane is None:
        raise PolicyError("ingest_reader_unsupported")
    return lane


def _record_identity(conn, message_id, table="conversation_messages"):
    if table == "ai_chat_messages":
        return _ai_chat_record_identity(conn, message_id)
...
def _ai_chat_record_identity(conn, message_id):
    """The stored AI-chat row, its parent's owner binding and a content revision.

    An AI-chat row carries no owner or dataset column: its conversation does.
    Re-binding that parent, moving the row to another conversation, changing its
    role or replacing its body therefore all stop the link from matching.
    Operational columns a routine sync rewrites (ingestion time, batch) are left out.
    """
    columns = ("message_id", "conversation_id", "sender_type", "sender_id", "event_at", "content_rendered",
               "metadata_json", "sequence", "source_id", "source_record_id", "actor_role")
    rows = conn.execute(f"SELECT {','.join(columns)},content FROM ai_chat_messages WHERE message_id=?", (message_id,)).fetchmany(2)
    if len(rows) != 1 or type(rows[0][-1]) is not str:
        raise PolicyError("ingest_canonical_collision")
    row = dict(zip(columns, rows[0][:-1]))
    parents = conn.execute("SELECT conversation_id,owner_user_id,source_id FROM ai_chat_conversations WHERE conversation_id=?",
                           (row["conversation_id"],)).fetchmany(2)
    if len(parents) != 1:
        raise PolicyError("ingest_canonical_collision")
    try:
        content_revision = hashlib.sha256(rows[0][-1].encode("utf-8")).hexdigest()
    except UnicodeError:
        raise PolicyError("ingest_canonical_collision") from None
    return digest({"table": "ai_chat_messages", "row": row, "content_revision": content_revision,
                   "conversation": dict(zip(("conversation_id", "owner_user_id", "source_id"), parents[0]))})
...
class VerifiedIngestContext:
    ...
    def record_insert(self, conn, message_id):
        ...
        conn.execute("INSERT INTO ingest_provenance_records VALUES (?,?,?,?,?)", (message_id, self.enrollment_id, self.enrollment_revision, self.job_id, _record_identity(conn, message_id, self.table)))
    ...
    def existing_record(self, conn, message_id):
        ...
        if row is None:
            return False
        if tuple(row) != (self.enrollment_id, self.enrollment_revision, self.job_id, _record_identity(conn, message_id, self.table)):
            raise PolicyError("ingest_canonical_collision")
        return True
...
class IngestProvenanceService:
    ...
    def _snapshot(self, snapshot_id, reader_contract=READER_CONTRACT):
        ...
        if ":" in snapshot_id or ".." in snapshot_id:
            raise PolicyError("ingest_identifier_invalid")
        path = self.root / (snapshot_id + lane.suffix)
        try:
            ...
            try:
                before = os.fstat(fd)
                # A SQLite file has a 100-byte header; the smallest export is "[]".
                smallest = 100 if lane.suffix == ".db" else 2
                if not stat.S_ISREG(before.st_mode) or stat.S_IMODE(before.st_mode) != 0o400 or before.st_nlink != 1 or not smallest <= before.st_size <= MAX_SNAPSHOT_BYTES:
                ...
                os.close(fd)
            if len(data) != before.st_size or (lane.suffix == ".db" and (data[:16] != b"SQLite format 3\x00" or data[18:20] != b"\x01\x01")):
                raise PolicyError("ingest_snapshot_not_closed")
            ...
            descriptor = {"snapshot_id": snapshot_id, "snapshot_sha256": hashlib.sha256(data).hexdigest(), "snapshot_bytes": len(data), "reader_contract": lane.reader_contract, "ownership_basis": OWNERSHIP_BASIS}
            return descriptor, data
        except OSError:
            raise PolicyError("ingest_snapshot_unavailable") from None

    def describe_snapshot(self, conn, *, snapshot_id, reader_contract=READER_CONTRACT):
        _owner(self.binding)
    ...
    def _enrollment(self, conn, enrollment_id, *, active=False, source_id=None):
        ...
        if row is None or lane is None or row[6] != lane.attestation or (source_id is not None and source_id != lane.source_id):
        ...
        if active:
            self._source_enabled(conn, row[2], lane.source_id)
        return dict(zip(("enrollment_id", "snapshot_json", "dataset_id", "revision", "state", "source_generation", "attestation"), row)) | {"lane": lane}

    @staticmethod
    def _source_enabled(conn, dataset_id, source_id="imessage"):
        ...
        if found[0] != "table" or not {"dataset_id", "source_id", "enabled"} <= {row[1] for row in conn.execute("PRAGMA table_info(user_ingestion_sources)")}:
            raise PolicyError("ingest_source_schema_invalid")
        rows = conn.execute("SELECT enabled FROM user_ingestion_sources WHERE source_id=? AND dataset_id=?", (source_id, dataset_id)).fetchmany(2)
    ...
    def enroll(self, conn, *, snapshot_id, dataset_id, snapshot_sha256, owner_attestation, reader_contract=READER_CONTRACT):
        ...
        self._connection(conn)
        self._source_enabled(conn, dataset_id, lane.source_id)
        snapshot, _ = self._snapshot(snapshot_id, lane.reader_contract)
        ...
        with self._transaction(conn):
            ...
            conn.execute("INSERT INTO ingest_provenance_enrollments VALUES(?,?,?,1,'active',?,?,?,?)", (enrollment_id, _json(snapshot), dataset_id, generation, lane.attestation, int(time.time()), current_principal().channel))
            row = self._enrollment(conn, enrollment_id, active=True)
        return self._enrollment_metadata(row)

    def revoke(self, conn, *, enrollment_id, source_id="imessage"):
        _owner(self.binding)
        with self._transaction(conn):
    ...
    def enqueue(self, conn, *, enrollment_id, source_id="imessage"):
        _owner(self.binding)
        with self._transaction(conn):
            row = self._enrollment(conn, enrollment_id, active=True, source_id=source_id)
            expected = _read_json(row["snapshot_json"])
            if self._snapshot(expected["snapshot_id"], expected["reader_contract"])[0] != expected:
                raise PolicyError("ingest_snapshot_changed")
    ...
    def status(self, conn, *, job_id, source_id="imessage"):
        _owner(self.binding)
        self._check(conn)
        metadata = self._job_metadata(conn, job_id)
        try:
            self._enrollment(conn, metadata["enrollment_id"], source_id=source_id)
        except PolicyError:
            raise PolicyError("ingest_job_unknown") from None
        return metadata

    def claim(self, conn, job_id, source_id="imessage"):
        with self._transaction(conn):
            self._check(conn)
            ...
            if row is None:
                raise PolicyError("ingest_job_unknown")
            try:
                self._enrollment(conn, row[0], source_id=source_id)
            except PolicyError:
                # Each runner claims only its own lane's jobs.
                raise PolicyError("ingest_job_unknown") from None
            ...
            context = VerifiedIngestContext(self, job_id, row[0], row[1], self.binding.owner_id, lane.source_id, enrollment["dataset_id"], token, lane.table)
    ...
    def assert_current(self, conn, context, *, source_id, dataset_id, _allow_done=False):
        ...
        if (source_id, dataset_id, context.owner_id) != (context.source_id, context.dataset_id, self.binding.owner_id):
            raise PolicyError("ingest_context_invalid")
        enrollment = self._enrollment(conn, context.enrollment_id, active=True, source_id=context.source_id)
        if enrollment["lane"].table != context.table:
            raise PolicyError("ingest_context_invalid")
    ...
    def snapshot_bytes(self, conn, context):
        ...
        actual, data = self._snapshot(expected["snapshot_id"], expected["reader_contract"])
        if expected != actual:
            raise PolicyError("ingest_snapshot_changed")
        return data

    def validate_record_origin(self, conn, *, message_id, origin, table="conversation_messages"):
        ...
        or source replacement withholds already-reviewed outputs as well. The
        link is valid only for the table its enrollment's reader writes.
        """
        ...
        if enrollment["lane"].table != table:
            raise PolicyError("ingest_origin_invalid")
        ...
        if link is None or tuple(link) != (enrollment["enrollment_id"], enrollment["revision"], origin["job_id"], _record_identity(conn, message_id, table)):
            raise PolicyError("ingest_origin_invalid")
        snapshot = _read_json(enrollment["snapshot_json"])
        if self._snapshot(snapshot["snapshot_id"], snapshot["reader_contract"])[0] != snapshot:
            raise PolicyError("ingest_snapshot_changed")
