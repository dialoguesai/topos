...
async def handle_control_plane_request(message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if msg_type == "list_home_chat_sessions":
        ...
        rows = hc_store.list_sessions(conn, user_id=user_id, engine_id=engine_id)
        return {"id": req_id, "status": "ok", "payload": {"sessions": rows}}

    if msg_type == "get_home_chat_session":
        from ..home_chat import store as hc_store
        ...
        row = hc_store.get_session(conn, user_id=user_id, session_id=session_id)
        if not row:
            return {"id": req_id, "status": "error", "error": "Session not found"}
        return {"id": req_id, "status": "ok", "payload": row}

    if msg_type == "upsert_home_chat_session":
        from ..home_chat import store as hc_store
    ...
    if msg_type == "delete_home_chat_session":
        from ..home_chat import store as hc_store
