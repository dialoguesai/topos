...
class TestAntiAliasRegression:
    ...
    def test_alias_pair_with_near_identical_contexts_still_gets_no_edge(
        ...
        ``Sierra ↔ Sierra Yankee`` at 0.682 and ``Sierra Yankee ↔ draftin1`` at
        ...
        """
        _add_person(conn, "sierra", "Sierra")
        _add_person(conn, "sierra_yankee", "Sierra Yankee")
        _add_person(conn, "other", "Devi Raman")
        _add_centroid(conn, "sierra", _unit({0: 1.0}))
        _add_centroid(conn, "sierra_yankee", _tilted(1, 0.99))
        ...
        active = _active_pairs(conn)
        assert ("sierra", "sierra_yankee") not in active
