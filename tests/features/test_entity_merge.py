...
def test_merge_clears_no_bind_so_rebind_works(conn):
    ...
    keep = r._create_entity("Claire Duncombe", "person", contact_id="c-cd")
    for i in range(2):
    ...
    # Guard is active: Claire must not resolve back to Claire Duncombe.
    hit, _ = EntityResolver(conn).resolve("Claire", entity_type="person")
    assert hit != keep

    merge_entity_pair(conn, keep, absorb)

    # Guard cleared: Claire can bind to Claire Duncombe again.
