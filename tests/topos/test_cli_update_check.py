...
def test_update_check_installs_when_user_confirms(monkeypatch):
    monkeypatch.setattr(runtime_update, "get_installed_package_version", lambda _: "0.1.0")
    monkeypatch.setattr(runtime_update, "get_latest_pypi_version", lambda _: "0.1.1")
...
def test_update_check_noop_when_up_to_date(monkeypatch):
    monkeypatch.setattr(runtime_update, "get_installed_package_version", lambda _: "0.1.1")
    monkeypatch.setattr(runtime_update, "get_latest_pypi_version", lambda _: "0.1.1")
    monkeypatch.setattr(
        runtime_update.subprocess,
        "run",
