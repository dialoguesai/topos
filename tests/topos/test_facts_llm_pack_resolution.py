"""PLAN_MODEL_PACKS.md M3: facts-LLM model resolution consults the active pack.
...
def test_device_override_wins_over_pack():
    ...
    _seed_pack(conn, provider="ollama", model="phi3:latest")
    conn.execute(
        "INSERT INTO engine_config (key, value) VALUES ('facts_llm_model', 'device-pin:latest')"
    )
    conn.commit()
    assert resolve_facts_llm_model(_Settings(), conn) == "device-pin:latest"
...
def test_no_connection_falls_back_to_settings_chain():
    assert resolve_facts_llm_model(_Settings(), None) == "extraction-default:latest"
