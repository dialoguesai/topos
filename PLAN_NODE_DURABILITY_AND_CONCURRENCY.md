...
| M2.4 rebuild compute out of process — gate-clean rebuild still starved the loop ~103s (2026-08-08, zero WRITE_GATE warnings): the worker thread's CPU (goal embeddings on MPS, role map, Louvain) held the GIL, an IO-vs-CPU thread convoy. File-backed rebuilds now spawn `python -m topos.features.entities.rebuild_subprocess`; advisory flock = cross-process single-flight (second rebuild defers like a derivation), WAL busy_timeout covers the child's short write phases; in-memory/kill-switch falls back in-process. Regression: loop-tick sampling during a real child rebuild | done 2026-08-08 | `features/entities/rebuild_subprocess.py`, `features/entities/graph_refresh.py`, `api/signal.py`, `upgrades/runner.py`, `tests/features/test_graph_rebuild_subprocess.py` |
